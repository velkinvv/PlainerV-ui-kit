 ...
 * Определяет все стили прогресс-баров для темной темы
 */
export const darkProgressTheme: ProgressTheme = {
  // Размеры для линейного прогресса (аналогично светлой теме)
  linearSizes: {
    [Size.XS]: {
      height: '2px',
      borderRadius: '2px',
  ...
  // Размеры для кругового прогресса (аналогично светлой теме)
  circularSizes: {
    [Size.XS]: {
      size: 40,
      thickness: 3,
  ...
  colors: {
    ...
    statusError: colors.red[400], // Цвет прогресса в статусе ошибки
  },

  // Типографика (аналогично светлой теме)
  typography: {
    label: {
      fontSize: '14px',
      fontWeight: 500,
