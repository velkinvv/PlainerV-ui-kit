...
**Текущая версия:** `0.3.3` · ветка [`v_0.3.3`](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.3.3) · React 18+/19 · styled-components 6.x
...
npm i @velkinvv/plainerv@0.3.3 react react-dom styled-components framer-motion
```
...
Подробнее: [документация Theming](https://github.com/velkinvv/PlainerV-ui-kit/blob/v_0.3.3/documentation/content/docs/ru/web/v_0.3.3/theming.mdx) (на сайте: **Web → v0.3.3 → Theming**).
...
**Полный перечень публичных экспортов** — в [документации](https://github.com/velkinvv/PlainerV-ui-kit/blob/v_0.3.3/documentation/content/docs/ru/web/v_0.3.3/components-catalog.mdx) (на сайте: **Web → v0.3.3 → Справочник компонентов**). Ниже — краткая группировка.
...
- Кнопки, SegmentedControl и навигация календаря: высота как у Input того же size, радиус из `theme.borderRadius`.
- **DateInput:** `precision` — день, месяц, год или неделя месяца (`weekOfMonthMode`: календарь / семёрки).
...
- Всплывающие панели (DateInput, Dropdown, Select, Popover, FloatingMenu) при первом открытии появляются сразу под якорем, без вылета из левого верхнего угла.
...
- **DateInput / TimeInput / DateTimeInput:** дефолтный `label` как у Input; `labelVariant="floating"` — прежний визуал. `padding-top: 10px` только в floating при наличии лейбла.
...
- **Alert:** `actionPlacement`, `action` и `onClose` вместе.
- **FloatButton:** плавающая кнопка, Group и BackTop.
...
- **Chip** / **Chips**, **List**, **DropMenu**, **MultiButton**, **Pulse**, **SegmentedControl**.
- **Storybook:** категория **Buttons** (после Inputs); Accordion — в Data Display.
- Гайды: Chip + List, DropMenu + MultiButton, SegmentedControl + Pulse.
...
- **Badge:** motion-анимация контента; **ValueMotion** — пульсирующее значение.
...
- **Темизация:** оси **ThemeVariant** × **ThemeColorScheme**; **ThemeVariantSelector**; переключение палитры без смены варианта оформления.
- **Детские темы:** `kidsBoys` и `kidsGirls` (8–11 лет), светлая и тёмная палитра; legacy `kids` → `kidsBoys`.
- **Storybook:** два переключателя в toolbar (Тема + Цвет), Docs/canvas для kidsBoys/kidsGirls.

Подробности — в [CHANGELOG.md](CHANGELOG.md).

## 📋 Что нового в 0.2.4

- **Carousel:** autoplay, parallax, overlay/overlayPanel, fullscreen, thumbnails, drag/swipe и эффекты слайдов.
- **DateTimeInput:** поле даты и времени; расширены **DateInput**, **TimeInput**, **Calendar** (picker).
- **ActionBar:** панель действий с overflow-меню.
- **Glass-темизация:** Accordion, Avatar, Badge, Hint, Modal, Pagination, Pill, Snackbar, Stepper, Tabs, Tag, Toast, Tooltip.
- **Table / DataGrid:** цвета строк, доработки сторис.
...
- [Ветка v0.3.3](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.3.3)
- [Ветка v0.3.2](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.3.2)
- [Ветка v0.3.1](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.3.1)
- [Ветка v0.3.0](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.3.0)
...
- [Ветка v0.2.5](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.2.5)
- [Ветка v0.2.4](https://github.com/velkinvv/PlainerV-ui-kit/tree/v_0.2.4)
